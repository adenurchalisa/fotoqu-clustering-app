...
const people = [
  { name: "Person A", count: 47, color: "#C9843A", photos: ["family-1", "family-3", "event-2"] },
  { name: "Person B", count: 23, color: "#7A9E9F", photos: ["family-2", "event-1", "portrait-1"] },
  { name: "Person C", count: 31, color: "#9B7A6E", photos: ["portrait-2", "event-3", "family-4"] },
  { name: "Person D", count: 12, color: "#6E7A9B", photos: ["event-4", "family-5"] },
];
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            >
              One folder per person.<br />
              <em style={{ color: "#C9843A" }}>Every photo accounted for.</em>
            </h2>
            <p
              style={{
            ...
            >
              Whether it's a family reunion, wedding, or years of phone photos — FotoQu organises chaos into a clean, browsable structure in minutes.
            </p>
            ...
            >
              <div style={{ color: "#C9843A" }}>📁 sorted_photos.zip</div>
              <div className="ml-4">├── Person A (47 photos)</div>
              <div className="ml-4">├── Person B (23 photos)</div>
              <div className="ml-4">├── Person C (31 photos)</div>
              <div className="ml-4">└── Person D (12 photos)</div>
            </div>

            <button
              className="mt-6 flex items-center gap-2 px-5 py-3 rounded-full transition-all duration-200"
              style={{
          ...
          <div className="grid grid-cols-2 gap-4">
            {people.map((person, i) => (
              <motion.div
                ...
                className="rounded-xl p-4 cursor-pointer transition-all duration-250"
                style={{
                  background: "rgba(247,245,240,0.04)",
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-1 mb-3">
                  {unsplashPhotos.slice(i * 1, i * 1 + 3).map((url, j) => (
                    <div
                      key={j}
